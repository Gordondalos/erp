...
if ($_FILES['upload']) {
    if (($_FILES['upload'] == "none") OR (empty($_FILES['upload']['name']))) {
        ...
        move_uploaded_file($_FILES['upload']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . "/images/" . $name);
        $full_path = $_SERVER['DOCUMENT_ROOT'] . '/images/' . $name;
        $message = "Файл " . $_FILES['upload']['name'] . " загружен";
        $size = @getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/' . $name);
    ...
    echo '<script type="text/javascript">window.parent.CKEDITOR.tools.callFunction("' . $callback . '", "/images/' . $name . '", "' . $message . '" );</script>';
}
?>